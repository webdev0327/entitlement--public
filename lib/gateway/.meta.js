 ...
 * @property {String[]} currentRoles - What you currently have
 * @property {String[]=} requiredRoles - What you would need to access
 */
...
function widgetAuthorizationObserver(operation, authorized, details) {
	...
	if (!authorized) {
		if (details.requiredRoles.some(r => r === 'registered') {
			showRegisteredModal();
		} else if (details.requiredRoles.some(r => r === 'subscriber') {
			showSubscriberModal();
		} else if (details.requiredRoles.count === 0) {
			showHelpModal();
		}
	}
}
