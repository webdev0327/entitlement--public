...
module.exports = (() => {
	...
	class EntitlementsGateway extends Disposable {
			...
			return Promise.resolve()
				.then(() => {
					...
					if (this._authorizationObserver !== null) {
						const permissions = this._entitlements.permissions;

						authorized = permissions.find(p => p.operation === operation) || false;

						this._authorizationObserver(operation, authorized, { type: 'modal', parameters: { type: 'REGISTER' } });
					}
		 ...
		 *
		 * @public
		 * @returns {Promise<Schema.EntitlementsServiceMetadata>}
		 */
		readServiceMetadata() {
			return Promise.resolve()
				.then(() => {
		 ...
		 *
		 * @public
		 * @returns {Promise<Schema.UserEntitlements>}
		 */
		readUser() {
			return Promise.resolve()
				.then(() => {
