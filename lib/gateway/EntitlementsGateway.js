...
module.exports = (() => {
					...
					if (this._startPromise === null) {
						this._startPromise = Promise.resolve()
							.then(() => {
								...
								this._authorizationObserver = authorizationObserver || null;

								return getAuthorizer.call(this).then(() => {
									return this;
								});
							}).catch((e) => {
								this._started = false;
	...
	function getAuthorizer() {
		return Promise.resolve()
			.then(() => {
				if (this._authorizerCachePromise === null || this._authorizerCachePromise === null || getTimestamp() > this._authorizerCacheTimestamp + REFRESH_INTERVAL_MILLISECONDS) {
	...
	class Authorizer {
		...
		authorize(operation, data) {
			...
			// Identify user permissions for operation.
			// If user has no permissions, reject authorization.
			// If the user has some permissions, and no permission has a restriction, accept the authorization.
			// If the user has some permissions, and all permissions have a rate-limit, deter the authorization to the server (by returning null).
			// If the user has some permissions, and all permissions have a count-limit, check the count limit (and accept or deny based on exceeded count).
